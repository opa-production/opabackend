...
class PaymentMethod(Base):
    ...
    )  # Nullable to support clients
    client_id: Mapped[int] = mapped_column(
        ForeignKey("clients.id"), nullable=True, index=True
    )  # Nullable to support hosts
    ...
    # For M-Pesa
    mpesa_number: Mapped[str] = mapped_column(
...
class Payment(Base):
    ...
    extension_request_id: Mapped[int] = mapped_column(
        ForeignKey("booking_extension_requests.id"), nullable=True, index=True
    )
    ...
    result_code: Mapped[Optional[int]] = mapped_column(
        nullable=True
    )  # Safaricom ResultCode
    result_desc: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True
    )  # e.g. "Insufficient funds", "User cancelled"
    ...
    mpesa_transaction_date: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
    # Pesapal: Card payment tracking (Visa/Mastercard)
    pesapal_order_tracking_id: Mapped[Optional[str]] = mapped_column(
        String(255), unique=True, nullable=True, index=True
    )
    pesapal_merchant_reference: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True
    )
    pesapal_confirmation_code: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )
    pesapal_payment_method: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # e.g., "Visa", "Mastercard"
    pesapal_payment_account: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # Last 4 digits
    # Ardena Pay: Stellar transaction hash when payment was made via USDC
    stellar_tx_hash: Mapped[Optional[str]] = mapped_column(
    ...
    extension_request: Mapped["BookingExtensionRequest"] = relationship(
        foreign_keys=[extension_request_id]
    )
...
class Host(Base):
    ...
    id_number: Mapped[str] = mapped_column(
        String(100), nullable=True
    )  # ID number, passport number, or DL number
    city: Mapped[str] = mapped_column(
        String(100), nullable=True
    )  # City where the host operates
    ...
    terms_accepted_at = mapped_column(
        DateTime(timezone=True), nullable=True
    )  # When user accepted T&C
    ...
    host_ratings: Mapped[list["HostRating"]] = relationship(
        back_populates="host", cascade="all, delete-orphan"
    )
    # Relationship to client (renter) ratings given by this host
    client_ratings: Mapped[list["ClientRating"]] = relationship(
        back_populates="host", cascade="all, delete-orphan"
    )
    ...
    host_kycs: Mapped[list["HostKyc"]] = relationship(
        back_populates="host", cascade="all, delete-orphan"
    )
...
class HostKyc(Base):
    ...
    veriff_session_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True
    )  # Veriff verification/session ID
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True
    )  # approved, declined, pending, resubmission_requested
...
class Withdrawal(Base):
    ...
    payment_details: Mapped[str] = mapped_column(
        Text, nullable=True
    )  # JSON: e.g. {"mpesa_number":"254..."} or {"bank_name":"...","account_number":"..."}

    # Payhero/M-Pesa B2C callback fields
    checkout_request_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True
    )  # Payhero TransactionID
    ...
    processed_by_admin_id: Mapped[int] = mapped_column(
...
class Client(Base):
    ...
    mobile_number: Mapped[str] = mapped_column(
        String(50), nullable=True
    )  # Required for updates, but nullable for existing clients
    id_number: Mapped[str] = mapped_column(
        String(100), nullable=True
    )  # Required for updates, but nullable for existing clients
    ...
    gender: Mapped[str] = mapped_column(
        String(20), nullable=True
    )  # Required for updates, but nullable for existing clients - e.g., "male", "female", "other"
    ...
    # KYC (Veriff) - one-to-many for history; use latest for status
    client_kycs: Mapped["ClientKyc"] = relationship(
        back_populates="client", cascade="all, delete-orphan"
    )
    # Biometric device tokens for local unlock
...
class ClientWallet(Base):
    ...
    network: Mapped[str] = mapped_column(
        String(20), nullable=False, default="testnet"
    )  # testnet | mainnet
    stellar_public_key: Mapped[str] = mapped_column(
        String(56), nullable=False, unique=True, index=True
    )
    stellar_secret_encrypted: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True
    )  # optional: encrypted or plain for testnet
    # Cached balances (updated on each GET /client/wallet from Horizon)
    balance_xlm: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, default="0"
    )
    balance_usdc: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, default="0"
    )
    balance_updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
...
class StellarPaymentTransaction(Base):
    ...
    amount_ksh: Mapped[float] = mapped_column(Float, nullable=False)
    amount_usdc: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # USD equivalent for display
    amount_xlm: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # Set when paid in XLM
    ...
    booking: Mapped["Booking"] = relationship(
        "Booking", back_populates="stellar_payments"
    )
    client: Mapped["Client"] = relationship(
        "Client", backref="stellar_payment_transactions"
    )
...
class IncomingStellarPayment(Base):
    ...
    client: Mapped["Client"] = relationship(
        "Client", backref="incoming_stellar_payments"
    )
...
class DrivingLicense(Base):
    ...
    # License information
    license_number = mapped_column(
        String(50), nullable=False, unique=True, index=True
    )  # Mix of letters and numbers
    category = mapped_column(
        String(10), nullable=False
    )  # One letter + number (e.g., B1, C2, D1)
...
class Car(Base):
    ...
    features: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # JSON string for up to 12 features
    ...
    image_urls: Mapped[str] = mapped_column(
        Text, nullable=True
    )  # JSON array of image URLs (legacy, kept for backward compatibility)
    ...
    car_images: Mapped[str] = mapped_column(
        Text, nullable=True
    )  # JSON array of up to 12 car image URLs
    ...
    verification_status: Mapped[str] = mapped_column(
        String(20), default=VerificationStatus.AWAITING.value, nullable=False
    )
...
class Booking(Base):
    ...
    )  # Human-readable ID like BK-12345678
    ...
    drive_type: Mapped[str] = mapped_column(
        String(20), default="self"
    )  # 'self' or 'withDriver'
    check_in_preference: Mapped[str] = mapped_column(
        String(20), default="self"
    )  # 'self' or 'assisted'
    ...
    pickup_reminder_sent_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    client_deleted_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime(timezone=True),
...
class BookingExtensionRequest(Base):
    ...
    extra_amount = mapped_column(
        Float, nullable=False
    )  # Includes base + damage waiver for extra_days
...
class BookingIssue(Base):
    ...
    issue_type = Column(
        String(50), nullable=False, index=True
    )  # e.g. damage, late_return, no_show, misconduct, other
...
Client.wallet = relationship(
    "ClientWallet", back_populates="client", uselist=False, cascade="all, delete-orphan"
)
# Update Client model to include host ratings relationship
Client.host_ratings = relationship(
    "HostRating", back_populates="client", cascade="all, delete-orphan"
)
# Update Client model to include ratings received from hosts
Client.client_ratings = relationship(
    "ClientRating", back_populates="client", cascade="all, delete-orphan"
)
...
class HostRating(Base):
    ...
    client = relationship(
        "Client", back_populates="host_ratings", foreign_keys=[client_id]
    )
    booking = relationship("Booking", foreign_keys=[booking_id])
...
class ClientRating(Base):
    ...
    # Relationships
    client = relationship(
        "Client", back_populates="client_ratings", foreign_keys=[client_id]
    )
...
class Notification(Base):
    ...
    sender_name = mapped_column(
        String(255), nullable=False, default="[Deon,CEO ardena]"
    )
...
class SupportConversation(Base):
    ...
    id = mapped_column(Integer, primary_key=True, index=True)
    host_id = mapped_column(
        Integer, ForeignKey("hosts.id"), nullable=False, unique=True, index=True
    )  # One conversation per host

    # Status
    status = mapped_column(
        String(20), default="open", nullable=False, index=True
    )  # open, closed
    is_read_by_host = mapped_column(
        Boolean, default=False, nullable=False
    )  # Host has read latest admin message
    is_read_by_admin = mapped_column(
        Boolean, default=False, nullable=False
    )  # Admin has read latest host message
...
class ClientHostConversation(Base):
    ...
    host = relationship("Host", foreign_keys=[host_id])
    messages = relationship(
        "ClientHostMessage", back_populates="conversation", cascade="all, delete-orphan"
    )
...
class ClientHostMessage(Base):
    ...
    id = mapped_column(Integer, primary_key=True, index=True)
    conversation_id = mapped_column(
        Integer, ForeignKey("client_host_conversations.id"), nullable=False, index=True
    )
