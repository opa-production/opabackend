...
SHOW_SECRET_TESTNET = os.getenv("STELLAR_SHOW_SECRET_TESTNET", "1").strip().lower() in (
    "1",
    "true",
    "yes",
)
...
@router.post(
    "/client/wallet", response_model=WalletResponse, status_code=status.HTTP_201_CREATED
)
def create_wallet(
    current_client: Client = Depends(get_current_client),
    db: Session = Depends(get_db),
...
@router.get(
    "/client/wallet/transactions", response_model=List[StellarTransactionResponse]
)
...
@router.get(
    "/client/wallet/incoming", response_model=List[IncomingWalletPaymentResponse]
)
def list_incoming_wallet_payments(
    ...
    existing_hashes = {
        row.stellar_tx_hash
        for row in db.query(IncomingStellarPayment.stellar_tx_hash)
    ...
    for pay in incoming_raw:
        ...
        if in_app_enabled:
            try:
                ...
                logger.exception(
                    "[WALLET] Failed to create notification for new incoming payment tx=%s: %s",
                    tx_hash[:16],
                    e,
                )
        ...
        try:
            ...
            for rec in missing:
                try:
                    ...
                    logger.exception(
                        "[WALLET] Failed to backfill notification for incoming_stellar_payment id=%s: %s",
                        rec.id,
                        e,
                    )
            if missing:
                logger.info(
                    "[WALLET] Backfilled %s notification(s) for client_id=%s",
                    len(missing),
                    current_client.id,
                )
        except Exception as e:
            logger.exception(
                "[WALLET] Backfill query failed for client_id=%s: %s",
                current_client.id,
                e,
            )
    ...
    if new_notif_count:
        logger.info(
            "[WALLET] Created %s in-app notification(s) for incoming payments, client_id=%s",
            new_notif_count,
            current_client.id,
        )
    db.commit()
