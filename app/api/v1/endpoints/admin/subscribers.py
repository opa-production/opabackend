...
from app.models import Admin, Subscriber
...
async def list_subscribers(
    ...
    subscribed_only: bool = Query(
        True, description="If true, only list currently subscribed emails"
    ),
...
async def get_subscriber_trends(
    ...
    unsub_q = (
        select(
            func.date(Subscriber.unsubscribed_at).label("d"),
            func.count(Subscriber.id).label("c"),
        )
        .filter(
            Subscriber.unsubscribed_at.isnot(None), Subscriber.unsubscribed_at >= since
        )
...
async def send_newsletter(
    ...
    for s in subscribers:
        background_tasks.add_task(
            send_email, s.email, request.subject, request.body_html
        )
