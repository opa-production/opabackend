...
from app.models import Booking, BookingStatus, Car, Client, Host, VerificationStatus
...
async def get_dashboard_stats(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    active_hosts_result = await db.execute(
        select(func.count(Host.id)).filter(Host.is_active.is_(True))
    )
    ...
    active_clients_result = await db.execute(
        select(func.count(Client.id)).filter(Client.is_active.is_(True))
    )
    ...
    visible_cars_result = await db.execute(
        select(func.count(Car.id)).filter(Car.is_hidden.is_(False))
    )
...
async def get_recent_activity(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    hosts_result = await db.execute(
        select(Host).order_by(Host.created_at.desc()).limit(10)
    )
    ...
    cars_result = await db.execute(
        select(Car).order_by(Car.created_at.desc()).limit(10)
    )
...
async def get_revenue_stats(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    result = await db.execute(
        select(func.sum(Booking.total_price)).filter(Booking.status.in_(paid_statuses))
    )
    money_in_val = result.scalar()
    money_in = float(money_in_val or 0)

    paid_count_result = await db.execute(
        select(func.count(Booking.id)).filter(Booking.status.in_(paid_statuses))
    )
