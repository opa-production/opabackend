...
def _collect_storage_files_recursive(
    bucket_name: str, base_path: str, max_depth: int = 4
) -> List[str]:
    """
...
async def list_cars(
    ...
    status: Optional[str] = Query(
        None, description="Filter by verification status (awaiting/verified/denied)"
    ),
    ...
    sort_by: Optional[str] = Query(
        "created_at", description="Sort field (id, name, model, year, created_at)"
    ),
    ...
    if search:
        search_filter = or_(
            Car.name.ilike(f"%{search}%"), Car.model.ilike(f"%{search}%")
        )
...
async def get_car_media(
    ...
    if listing_mode != "none":
        ...
        for folder in folder_candidates:
            ...
            try:
                if use_sdk:
                    collected_paths = _collect_storage_files_recursive(
                        bucket_name, folder
                    )
                else:
                    collected_paths = collect_storage_file_paths_http(
                        bucket_name, folder
                    )
    ...
    image_urls = list(
        dict.fromkeys([u for u in image_urls if isinstance(u, str) and u.strip()])
    )
    video_urls = list(
        dict.fromkeys([u for u in video_urls if isinstance(u, str) and u.strip()])
    )
...
async def hide_car(
    ...
    if car.is_hidden:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Car is already hidden"
        )
...
async def show_car(
    ...
    if not car.is_hidden:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Car is already visible"
        )
