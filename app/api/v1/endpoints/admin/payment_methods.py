...
from app.models import Host, PaymentMethod
from app.schemas import PaymentMethodListResponse, PaymentMethodResponse
...
def _format_payment_method_response(payment_method: PaymentMethod) -> dict:
    ...
    if payment_method.expiry_month and payment_method.expiry_year:
        ...
        response_data["expiry_date"] = (
            f"{payment_method.expiry_month:02d}/{year_short:02d}"
        )
    else:
...
async def list_payment_methods(
    ...
    method_type: Optional[str] = Query(
        None, description="Filter by method type (mpesa, visa, mastercard)"
    ),
    search: Optional[str] = Query(
        None, description="Search by payment method name or host name/email"
    ),
    sort_by: Optional[str] = Query(
        "created_at", description="Sort field (id, created_at, name)"
    ),
    ...
    return {"payment_methods": payment_method_list, **pagination}
...
async def get_payment_method_details(
    ...
    stmt = (
        ...
        .options(joinedload(PaymentMethod.host))
    ...
    pm_data["host_name"] = (
        payment_method.host.full_name if payment_method.host else None
    )
