...
async def list_feedback(
    ...
    sort_by: Optional[str] = Query(
        "created_at", description="Sort field (id, created_at)"
    ),
    ...
    return PaginatedFeedbackListResponse(feedbacks=feedback_list, **pagination)
...
async def get_feedback_details(
    ...
    stmt = (
        select(Feedback)
        .options(joinedload(Feedback.host))
...
async def unflag_feedback(
    ...
    if not feedback.is_flagged:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Feedback is not flagged"
        )
