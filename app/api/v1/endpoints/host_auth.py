...
from app.models import Host, HostBiometricToken, Notification
...
from app.services.email_welcome import send_email, send_welcome_email_host
...
async def login_host(
    request: Request, body: HostLoginRequest, db: AsyncSession = Depends(get_db)
):
    """
...
async def refresh_host_token(
    ...
    if not host:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Host no longer exists"
        )
...
async def forgot_password(
    ...
    if web_url:
        ...
        reset_link = (
            f"{request.url_for('host_password_reset_redirect')}?token={reset_token}"
        )
...
@router.get(
    "/host/auth/reset-password/redirect",
    response_class=HTMLResponse,
    name="host_password_reset_redirect",
)
