...
@router.get(
    "/host/subscription/payment-status",
...
async def host_subscription_payment_status(
    checkout_request_id: str = Query(
        ..., description="CheckoutRequestID from checkout response"
    ),
    current_host: Host = Depends(get_current_host),
    db: AsyncSession = Depends(get_db),
):
    ...
    if not rec:
        raise HTTPException(
            status_code=404, detail="No subscription payment found for this checkout id"
        )
