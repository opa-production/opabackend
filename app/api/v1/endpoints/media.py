...
from app.models import Car, Client, Host
...
async def upload_client_avatar(
    ...
    return MediaUploadResponse(
        success=True, url=result["url"], message="Avatar uploaded successfully"
    )
...
async def upload_client_document(
    ...
    old_url = (
        current_client.id_document_url
        if document_type == "id"
        else current_client.license_document_url
    )
    if old_url:
...
async def upload_host_cover_image(
    ...
    return MediaUploadResponse(
        success=True, url=result["url"], message="Cover image uploaded successfully"
    )
...
async def upload_host_document(
    ...
    # Delete old document if exists
    old_url = (
        current_host.id_document_url
        if document_type == "id"
        else current_host.license_document_url
    )
    if old_url:
        old_path = old_url.split("/")[-4:]  # Extract path from URL
...
@router.post(
    "/host/upload/vehicle/{car_id}/images", response_model=MultipleMediaUploadResponse
)
...
async def upload_vehicle_video(
    ...
    return MediaUploadResponse(
        success=True, url=result["url"], message="Vehicle video uploaded successfully"
    )
