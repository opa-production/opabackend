...
from app.schemas import (
    ClientKycSessionRequest,
    ClientKycSessionResponse,
...
async def client_kyc_redirect(
    return_to: Optional[str] = Query(
        None, description="Deep link to open the app, e.g. ardena://kyc/result"
    ),
):
    """
    ...
    try:
        ...
        if not r.ok:
            try:
                ...
                err_body = r.text or ""
            logger.warning(
                "Veriff client session create %s: %s", r.status_code, err_body
            )
        r.raise_for_status()
        data = r.json()
