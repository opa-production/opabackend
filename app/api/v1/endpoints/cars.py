...
def parse_image_urls(image_urls_str: Optional[str]) -> List[str]:
    ...
    try:
        ...
        if isinstance(urls, list):
            logger.debug(
                f"🖼️ [PARSE IMAGE URLS] Successfully parsed {len(urls)} URLs from JSON string"
            )
            return urls
        ...
        logger.warning(
            f"🖼️ [PARSE IMAGE URLS] Failed to parse JSON: {str(e)}, input={image_urls_str[:100] if image_urls_str else None}"
        )
        return []
...
def _car_to_response(db_car: Car) -> CarResponse:
    ...
    return CarResponse(
        ...
        verification_status=VerificationStatus(db_car.verification_status).value
        if db_car.verification_status
        else VerificationStatus.AWAITING.value,
        is_hidden=db_car.is_hidden,
        drive_setting=getattr(db_car, "drive_setting", None) or "self_only",
        allowed_drive_types=_get_allowed_drive_types(
            getattr(db_car, "drive_setting", None)
        ),
        ...
        image_urls=car_images_parsed
        if car_images_parsed
        else image_urls,  # Legacy - parsed array for backward compatibility
...
def car_to_listing_response(car: Car) -> dict:
    ...
    return {
        ...
        "allowed_drive_types": _get_allowed_drive_types(
            getattr(car, "drive_setting", None)
        ),
        # Host information
...
async def get_car_listings(
    ...
    return CarListResponse(cars=car_responses, total=total, skip=skip, limit=limit)
...
async def explore_cars(
    ...
    search: Optional[str] = Query(
        None, description="Search by car name, model, or location"
    ),
    location: Optional[str] = Query(None, description="Filter by location name"),
    min_price: Optional[float] = Query(None, ge=0, description="Minimum daily rate"),
    max_price: Optional[float] = Query(None, ge=0, description="Maximum daily rate"),
    ...
    """
    logger.info(
        f"🖼️ [EXPLORE CARS] Request received: page={page}, limit={limit}, "
    ...
    for car in cars:
        ...
        if car.cover_image:
            ...
            logger.debug(
                f"🖼️ [EXPLORE CARS] Car {car.id}: Using cover_image field: {cover_image}"
            )
        elif car.car_images:
            car_images = parse_image_urls(car.car_images)
            logger.debug(
                f"🖼️ [EXPLORE CARS] Car {car.id}: Parsed car_images: {car_images}"
            )
            if car_images and len(car_images) > 0:
                cover_image = car_images[0]
                cover_image_source = "car_images_first"
                logger.debug(
                    f"🖼️ [EXPLORE CARS] Car {car.id}: Using first from car_images: {cover_image}"
                )
            else:
                logger.warning(
                    f"🖼️ [EXPLORE CARS] Car {car.id}: car_images exists but parsed to empty list"
                )
        elif car.image_urls:
            # Legacy fallback
            image_urls = parse_image_urls(car.image_urls)
            logger.debug(
                f"🖼️ [EXPLORE CARS] Car {car.id}: Parsed legacy image_urls: {image_urls}"
            )
            if image_urls and len(image_urls) > 0:
                cover_image = image_urls[0]
                cover_image_source = "image_urls_legacy"
                logger.debug(
                    f"🖼️ [EXPLORE CARS] Car {car.id}: Using first from legacy image_urls: {cover_image}"
                )
            else:
                logger.warning(
                    f"🖼️ [EXPLORE CARS] Car {car.id}: image_urls exists but parsed to empty list"
                )

        if not cover_image:
            logger.warning(
                f"🖼️ [EXPLORE CARS] Car {car.id}: No cover image found. "
        ...
        logger.debug(
            f"🖼️ [EXPLORE CARS] Car {car.id}: Added to response. cover_image={cover_image} "
            f"(source={cover_image_source}), car_name={car_name}"
        )

    logger.info(
        f"🖼️ [EXPLORE CARS] ✅ Returning {len(car_responses)} cars. "
        f"Total matching: {total}, Page: {page}/{total_pages}"
    )

    return CarExploreListResponse(
        cars=car_responses, total=total, page=page, limit=limit, total_pages=total_pages
    )
...
@router.post(
    "/cars/basics", response_model=CarResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_car_media(
    ...
    """
    logger.info(
        f"🖼️ [UPDATE CAR MEDIA] Request received for car_id={car_id}, host_id={current_host.id}"
    )
    logger.info(
        f"🖼️ [UPDATE CAR MEDIA] Request body: files={request.files} ({len(request.files)} images), "
    ...
    logger.info(
        f"🖼️ [UPDATE CAR MEDIA] Car found: car_id={db_car.id}, owner_host_id={db_car.host_id}, "
    ...
    if db_car.host_id != current_host.id:
        logger.warning(
            f"🖼️ [UPDATE CAR MEDIA] Permission denied: car owner={db_car.host_id}, "
    ...
    updates["car_images"] = {
        "old": old_car_images,
        "new": db_car.car_images,
        "count": len(request.files),
    }
    logger.info(
        f"🖼️ [UPDATE CAR MEDIA] Updating car_images: {len(request.files)} images"
    )
    ...
    if request.cover_image:
        ...
        updates["cover_image"] = {
            "old": old_cover,
            "new": request.cover_image,
            "source": "provided",
        }
        logger.info(
            f"🖼️ [UPDATE CAR MEDIA] Updating cover_image (provided): {old_cover} -> {request.cover_image}"
        )
        ...
        updates["cover_image"] = {
            "old": old_cover,
            "new": request.files[0],
            "source": "auto_from_first_file",
        }
        logger.info(
            f"🖼️ [UPDATE CAR MEDIA] Auto-setting cover_image to first file: {request.files[0]}"
        )
    ...
    if request.car_video is not None:
        ...
        logger.info(
            f"🖼️ [UPDATE CAR MEDIA] Updating car_video: {old_video} -> {request.car_video}"
        )
    ...
    try:
        ...
        await db.refresh(db_car)
        logger.info(
            f"🖼️ [UPDATE CAR MEDIA] ✅ Successfully updated car media for car_id={car_id}. "
            f"Updates: {updates}"
        )
        logger.info(
            f"🖼️ [UPDATE CAR MEDIA] Final state: cover_image={db_car.cover_image}, "
            f"car_images={db_car.car_images}, car_video={db_car.car_video}"
        )
    except Exception as e:
        logger.error(
            f"🖼️ [UPDATE CAR MEDIA] ❌ Database commit failed: {str(e)}", exc_info=True
        )
        await db.rollback()
        raise HTTPException(
...
async def save_car_media_urls(
    ...
    """
    logger.info(
        f"🖼️ [SAVE IMAGE URLS API] Request received for car_id={car_id}, host_id={current_host.id}"
    )
    logger.info(
        f"🖼️ [SAVE IMAGE URLS API] Request body: files={request.files}, "
        f"cover_image={request.cover_image}, car_video={request.car_video}"
    )
    logger.info(
        f"🖼️ [SAVE IMAGE URLS API] Request body types: files_type={type(request.files)}, "
    ...
    logger.info(
        f"🖼️ [SAVE IMAGE URLS API] Car found: car_id={db_car.id}, owner_host_id={db_car.host_id}, "
        f"current_cover={db_car.cover_image}, current_car_images={db_car.car_images}, "
        f"current_video={db_car.car_video}"
    )
    ...
    if db_car.host_id != current_host.id:
        logger.warning(
            f"🖼️ [SAVE IMAGE URLS API] Permission denied: car owner={db_car.host_id}, "
            f"requesting host={current_host.id}"
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    logger.info(
        f"🖼️ [SAVE IMAGE URLS API] Field validation: has_files={has_files}, "
        f"has_cover={has_cover}, has_video={has_video}"
    )

    if not has_files and not has_cover and not has_video:
        logger.error(
            f"🖼️ [SAVE IMAGE URLS API] Validation failed: All fields are empty/null. "
    ...
    if request.files is not None:
        ...
        if request.files:
            ...
            logger.info(
                f"🖼️ [SAVE IMAGE URLS API] Updating car_images: {len(request.files)} images"
            )
            ...
            logger.warning(
                f"🖼️ [SAVE IMAGE URLS API] files array is empty, setting car_images to None"
            )
    ...
    if request.cover_image:
        ...
        logger.info(
            f"🖼️ [SAVE IMAGE URLS API] Updating cover_image (provided): {old_cover} -> {request.cover_image}"
        )
        ...
        logger.info(
            f"🖼️ [SAVE IMAGE URLS API] Auto-setting cover_image to first file: {request.files[0]}"
        )
    ...
    if request.car_video is not None:
        ...
        updates["car_video"] = {"old": old_video, "new": request.car_video}
        logger.info(
            f"🖼️ [SAVE IMAGE URLS API] Updating car_video: {old_video} -> {request.car_video}"
        )
    ...
    try:
        ...
        await db.refresh(db_car)
        logger.info(
            f"🖼️ [SAVE IMAGE URLS API] ✅ Successfully saved car media URLs for car_id={car_id}"
        )
        logger.info(
            f"🖼️ [SAVE IMAGE URLS API] Final state: cover_image={db_car.cover_image}, "
            f"car_images={db_car.car_images}, car_video={db_car.car_video}"
        )
        logger.info(f"🖼️ [SAVE IMAGE URLS API] Response data: {updates}")
    except Exception as e:
        logger.error(
            f"🖼️ [SAVE IMAGE URLS API] ❌ Database commit failed: {str(e)}",
            exc_info=True,
        )
        await db.rollback()
        raise HTTPException(
...
@cache(expire=300)  # Cache for 5 minutes
async def get_car_details(car_id: int, db: AsyncSession = Depends(get_db)):
    """
    ...
    if not car:
        logger.warning(
            f"🚗 [CAR DETAILS] Car not found or not available: car_id={car_id}"
        )
        raise HTTPException(
    ...
    logger.info(
        f"🚗 [CAR DETAILS] ✅ Returning car details for car_id={car_id}, "
...
async def get_car_status(car_id: int, db: AsyncSession = Depends(get_db)):
    """
    ...
    return CarStatusResponse(
        car_id=db_car.id, verification_status=verification_status.value
    )
...
async def delete_my_car(
    ...
    """
    result = await db.execute(
        select(Car).filter(Car.id == car_id, Car.host_id == current_host.id)
    )
    car = result.scalar_one_or_none()
...
async def _add_blocked_date_internal(
    ...
    return {
        ...
        "blocked_date": {
            ...
            "created_at": blocked_date_obj.created_at.isoformat()
            if blocked_date_obj.created_at
            else None,
        },
    }
...
async def get_car_blocked_dates(
    ...
    result = await db.execute(
        select(CarBlockedDate)
        .filter(CarBlockedDate.car_id == car_id)
        .order_by(CarBlockedDate.start_date)
    )
        ...
        "blocked_dates": [
            {
                ...
                if bd.blocked_date
                else bd.start_date.date().isoformat(),
...
async def add_car_blocked_date(
    ...
    """
    return await _add_blocked_date_internal(
        car_id, request.blocked_date, request.reason, current_host, db
    )
...
async def remove_car_blocked_date(
    ...
    result = await db.execute(
        select(CarBlockedDate).filter(
            CarBlockedDate.id == blocked_date_id, CarBlockedDate.car_id == car_id
        )
    )
