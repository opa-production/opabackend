...
from app.models import Booking, BookingStatus, Client, Host, HostRating
...
async def create_host_rating(
    ...
    if request.booking_id:
        booking_stmt = (
            ...
            .filter(
                Booking.id == request.booking_id, Booking.client_id == current_client.id
            )
        )
    ...
    rating_stmt = (
        select(HostRating)
        .options(joinedload(HostRating.client))
...
async def get_host_rating_details(
    ...
    stmt = (
        select(HostRating)
        .options(joinedload(HostRating.client))
        .filter(HostRating.id == rating_id, HostRating.client_id == current_client.id)
    )
    result = await db.execute(stmt)
...
async def update_host_rating(
    ...
    stmt = select(HostRating).filter(
        HostRating.id == rating_id, HostRating.client_id == current_client.id
    )
    result = await db.execute(stmt)
    rating = result.scalar_one_or_none()
...
@router.delete(
    "/client/host-ratings/{rating_id}", status_code=status.HTTP_204_NO_CONTENT
)
