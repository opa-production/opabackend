...
from app.models import Client, ClientHostConversation, ClientHostMessage, Host
...
def _message_to_response(
    db_message: ClientHostMessage, client: Client = None, host: Host = None
) -> ClientHostMessageResponse:
...
@router.post(
    "/client/messages/host/{host_id}",
    response_model=ClientHostMessageResponse,
...
@router.get(
    "/client/messages/host/{host_id}", response_model=ClientHostConversationResponse
)
async def get_conversation_with_host(
    ...
    msg_stmt = (
        ...
        .filter(ClientHostMessage.conversation_id == conversation.id)
...
async def get_client_conversations(
    ...
    stmt = (
        select(ClientHostConversation)
        .filter(ClientHostConversation.client_id == current_client.id)
    ...
    for conv in conversations:
        ...
        for msg in messages:
            message_list.append(
                _message_to_response(msg, client=current_client, host=host)
            )
...
@router.post(
    "/host/messages/client/{client_id}",
    response_model=ClientHostMessageResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.get(
    "/host/messages/client/{client_id}", response_model=ClientHostConversationResponse
)
...
async def get_host_conversations(
    ...
    stmt = (
        select(ClientHostConversation)
        .filter(ClientHostConversation.host_id == current_host.id)
        .order_by(ClientHostConversation.last_message_at.desc())
    )
    result = await db.execute(stmt)
    conversations = result.scalars().all()
    ...
    for conv in conversations:
        ...
        for msg in messages:
            message_list.append(
                _message_to_response(msg, client=client, host=current_host)
            )
