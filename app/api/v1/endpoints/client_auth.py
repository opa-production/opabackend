...
async def register_client(
    ...
    db_client = Client(
        full_name=body.full_name, email=body.email, hashed_password=hashed_password
    )
    ...
    try:
        ...
        from app.services.stellar_wallet import _is_testnet, create_and_fund_wallet
        ...
        logging.getLogger(__name__).warning(
            "Ardena Pay wallet creation failed for new client: %s", e
        )
...
async def login_client(
    request: Request, body: ClientLoginRequest, db: AsyncSession = Depends(get_db)
):
    """
...
async def forgot_password(
    ...
    if web_url:
        ...
        base_url = (
            settings.PASSWORD_RESET_LINK_BASE_URL
            or settings.FRONTEND_URL
            or "https://ardena.co.ke"
        ).strip()
...
async def refresh_client_token(
    ...
    if not client:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Client no longer exists"
        )
...
async def export_client_data(
    ...
    result = await db.execute(
        select(PaymentMethod).filter(PaymentMethod.client_id == current_client.id)
    )
    payment_methods = result.scalars().all()

    result = await db.execute(
        select(ClientRating).filter(ClientRating.client_id == current_client.id)
    )
    ratings_from_hosts = result.scalars().all()

    result = await db.execute(
        select(HostRating).filter(HostRating.client_id == current_client.id)
    )
...
async def delete_own_account(
    ...
    result = await db.execute(
        select(BookingExtensionRequest).filter(
    ...
    await db.execute(
        delete(BookingExtensionRequest).filter(
            BookingExtensionRequest.client_id == client_id
        )
    )
    await db.execute(
        delete(ClientHostConversation).filter(
            ClientHostConversation.client_id == client_id
        )
    )
    await db.execute(
        delete(ClientFeedback).filter(ClientFeedback.client_id == client_id)
    )
    await db.execute(
...
@router.post(
    "/client/driving-license",
    response_model=DrivingLicenseResponse,
    status_code=status.HTTP_201_CREATED,
)
async def add_driving_license(
    ...
    db_license = DrivingLicense(
        client_id=current_client.id,
        license_number=request.license_number.upper().replace(
            " ", ""
...
async def get_driving_license(
    ...
    if not db_license:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No driving license found"
        )
...
@router.post(
    "/client/auth/biometric-login", response_model=ClientLoginResponseWithRefresh
)
async def biometric_login(
    ...
    result = await db.execute(
        select(ClientBiometricToken).filter(
